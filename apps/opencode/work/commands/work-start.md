...
5. Delegate to `@plan` to produce `docs/plans/<topic>.md` unless a plan already exists
...
9. Hand off to `@orchestrate` with task ID, branch, plan path, and remaining plan tasks

Do not implement product code. Do not hand off without confirmed task ID, branch, and plan path.
