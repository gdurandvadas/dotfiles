...
if [[ "$provider" == "claude" ]]; then
  CONFIG="$HOME/.config/dotfiles/apps/opencode/config.claude.json"
else
  CONFIG="$HOME/.config/dotfiles/apps/opencode/config.json"
fi
...
ORIGINAL="$(readlink "$LINK" 2>/dev/null || echo "$HOME/.config/dotfiles/apps/opencode/config.json")"
ln -sf "$CONFIG" "$LINK"
